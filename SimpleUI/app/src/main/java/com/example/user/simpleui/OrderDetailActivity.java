...
public class OrderDetailActivity extends AppCompatActivity implements GeoCodingTask.GeoCodingResponse, GoogleApiClient.ConnectionCallbacks, GoogleApiClient.OnConnectionFailedListener, LocationListener, RoutingListener {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        TextView orederResultTextView = (TextView)findViewById(R.id.orderResultsTextView);
        ...
        orederResultTextView.setText(orderResultText);
    ...
    public void onConnected(Bundle bundle) {
        if (ActivityCompat.checkSelfPermission(this, android.Manifest.permission.ACCESS_FINE_LOCATION) != PackageManager.PERMISSION_GRANTED)
        {
            ...
            {
                requestPermissions(new String[]{android.Manifest.permission.ACCESS_FINE_LOCATION}, ACCESS_FINE_LOCATION_REQUEST_CODE);
            }
            return;
        }
    ...
    public void onLocationChanged(Location location) {
        ...
        for (int i=0; i < points.size();i ++)
        {
            if (i <= points.size() -1)
            {
                ...
                if (location.getLatitude() >= minLat && location.getLatitude() <= maxLat && location)
                {
                    index = i;
                    break;
                }
